...
echo "$($PSQL "SELECT DISTINCT name FROM teams INNER JOIN games ON teams.team_id IN (games.winner_id, games.opponent_id) WHERE games.year = 2014 AND games.round = 'Eighth-Final' ORDER BY name")"
